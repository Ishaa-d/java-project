...
stage('UnitTests'){
  ...
  git url: 'https://github.com/Ishaa-d/java-project.git'
   sh 'ant -f test.xml -v'
...
stage('Deploy'){
   sh 'aws s3 cp /workspace/java-pipeline/dist/rectangle-*.jar jenkins-stack-s3bucket-1lni2dkquioa6.s3.amazonaws'
}
stage('Report'){
    stage ('UnitTests'){
